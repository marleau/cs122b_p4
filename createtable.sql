...
delimiter $$
DROP TRIGGER IF EXISTS updateFT$$
CREATE TRIGGER updateFT BEFORE UPDATE ON movies
    FOR EACH ROW BEGIN
        UPDATE ft SET title=NEW.title WHERE movie_id=NEW.id;
    END;
$$

DROP TRIGGER IF EXISTS insertFT$$
CREATE TRIGGER insertFT BEFORE INSERT ON movies
    FOR EACH ROW BEGIN
        INSERT INTO ft VALUES(NEW.id, NEW.title, NEW.year);
    END;
$$
delimiter ;
